...
import { readAzureStorageConfigs } from './config';
import {
...
export class AzureBlobStorageEntityProvider implements EntityProvider {
  ...
  static fromConfig(
    ...
    const providerConfigs = readAzureStorageConfigs(configRoot);
    ...
    return providerConfigs.map(providerConfig => {
      const integration = ScmIntegrations.fromConfig(
        configRoot,
      ).azureBlobStorage.byHost(providerConfig.accountName);

      if (!integration) {
        throw new Error('No integration found for azureStorage');
      }
  ...
  private constructor(
    ...
    this.storageClient = new BlobServiceClient(
      `https://${integration?.config?.accountName}.blob.core.windows.net${integration?.config?.secretAccessKey}`,
    );
  ...
  private async listAllObjectKeys(): Promise<string[]> {
    ...
    if (this.config.containerName) {
      this.getKeysFromContainer(this.config.containerName, keys);
    } else {
      const containers = this.storageClient.listContainers();
      for await (const container of containers) {
        this.getKeysFromContainer(container.name, keys);
      }
    }
    return keys;
  }

  private async getKeysFromContainer(container: string, keys: string[]) {
    const containerClient: ContainerClient =
      this.storageClient.getContainerClient(container);
    const blobs = containerClient.listBlobsFlat({ prefix: this.config.prefix });
    for await (const blob of blobs) {
      keys.push(`${container}/${blob.name}`);
    }
  }
  ...
  private createObjectUrl(key: string): string {
    return encodeURI(
      `https://${this.integration?.config?.accountName}.blob.core.windows.net/${key}${this.integration.config.secretAccessKey}`,
    );
  }
}
